...
class SampleOperations
  class << self
    ...
    def calculate_r(arr1, arr2)
      calculate_cov(arr1, arr2) / Math.sqrt(calculate_var(arr1)*calculate_var(arr2))
    end

    def calculate_var(arr)
      arr.squares.mean - arr.mean**2
    end

    def calculate_cov(arr1, arr2)
      (arr1*arr2).mean - arr1.mean*arr2.mean
    end
