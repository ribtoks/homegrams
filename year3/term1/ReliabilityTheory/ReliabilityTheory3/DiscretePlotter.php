...
class DiscretePlotter
{
    ...
    public function ProcessGraphics()
    {
        ...
        $minValue = round($this->minXvalue);
        $maxValue = round($this->maxXvalue);
        ...
        for ($i = 0; $i < $dataCount; ++$i)
        {
            $widthsMax[] = round(max($this->arraysY[$i]));
            $widthsMin[] = round(min($this->arraysY[$i]));
        }
        ...
        for ($i = 0; $i < $dataCount; ++$i)
        {
            ...
            {
                $maxElement = round(max($arrMax));
                $minElement = round(min($arrMin));
        ...
        for ($i = 0; $i < $dataCount; ++$i)
        {
            ...
            $maxElement = round(max($this->arraysY[$i]));
            $currWidth = $maxElement - round(min($this->arraysY[$i]));
        ...
        $graphicColor, $background, $offsetX, $offsetY, $width, $height, $markerCode)
    {
        ...
        // xValues are sorted
        $minX = round($xData[0]);
        $maxX = round($xData[$xCount - 1]);

        $minY = round(min($yData));
        $maxY = round(max($yData));
